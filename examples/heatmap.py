...
(
    ...
    .style("stroke", "none")
    .style("opacity", 0.8)
    .on("mouseover", mouseover, extra_nodes=[tooltip.node()], html_nodes=[tooltip.node()])
    .on("mousemove", mousemove, extra_nodes=[tooltip.node()], html_nodes=[tooltip.node()])
    .on("mouseleave", mouseleave, extra_nodes=[tooltip.node()], html_nodes=[tooltip.node()])
)
html.create_app().run()
